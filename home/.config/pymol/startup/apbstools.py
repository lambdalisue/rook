...
def __init__(self):
    ...
    self.menuBar.addmenuitem('Plugin', 'command',
                             'Launch APBS Tools (Modified)',
                             label='APBS Tools (Modified)...',
...
"""
    ...
    def _generatePdb2pqrPqrFile(self):
        ...
        try:
            ...
            show_error((
                'Please set a temporary PDB file location that you have '
                'permission to edit'
            ))
            return False
        ...
        args = list(chain.from_iterable([
            [self.pdb2pqr.getvalue().strip() or "pdb2pqr"],
        ...
        if retval != 0:
            show_error((
                'Could not run pdb2pqr: %s\n\nIt returned %s.\nCheck the '
                'PyMOL external GUI window for more information\n'
                '-------------------------------------------------------\n'
                'stdout:\n%s\n'
                'stderr:\n%s\n'
            ) % (args, retval, stdout, stderr))
            return False
        self.cleanupGeneratedPdbOrPqrFile(
            self.pymol_generated_pqr_filename.getvalue()
        )
        unassigned_atoms = self.getUnassignedAtomsFromPqr(
            self.pymol_generated_pqr_filename.getvalue()
        )
        if unassigned_atoms:
            pymol.cmd.select('unassigned','ID %s'%unassigned_atoms)
            message_text = (
                "Unable to assign parameters for the %s atoms in selection "
                "'unassigned'.\n"
                "Please either remove these unassigned atoms and re-start the "
                "calculation\nor fix their parameters in the generated PQR "
                "file and run the calculation\nusing the modified PQR file "
                "(select 'Use another PQR' in 'Main')."
            ) % len(unassigned_atoms.split('+'))
